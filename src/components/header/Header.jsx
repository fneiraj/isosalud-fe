...
const Header = (props) => {
    ...
    return (
      <Tabs value={currentTab} textColor='inherit' variant='scrollable' scrollButtons='auto'>
        {pageProps.tabs.map(({ title, link }) => (
          <Tab
            key={title} textColor='inherit' label={title} component={Link}
  ...
  const SubTabs = ({ currentTab, subTabsCurrentPage }) => (
  ...
  const renderSubTabs = () => {
    ...
    return subTabsCurrentPage ? <SubTabs currentTab={currentTab} subTabsCurrentPage={subTabsCurrentPage} /> : null
  }
